<?php

function project_slide_shortcode($attr,$content = null){
	$a = extract(shortcode_atts(array(
		// 'title' => 'this is title',
		'posts_per_page'	=> 3,
		'cat_in'			=>''
		// 'show_pagination'=> 0,
	),$attr));
	ob_start();
	$master_class = apply_filters( 'kc-el-class', $attr );
	if (!function_exists('kc_add_map')) { 
		$master_class = array();
	}
	$master_class[] = ' project-carosel-section';
	$cat_in = str_replace(',', ' ', $cat_in);
	$cat_in_s = explode( " ", $cat_in );

	?>
	    <section class="<?php echo esc_attr( implode( ' ', $master_class ) ); ?>">
				<div class="post-curosel ">
 
			<?php
				// $paged = (get_query_var('page')) ? get_query_var('page') : 1;
			    $loop = new WP_Query( array( 
			    	'post_type' 		=> 'alone-project', 
			    	'posts_per_page' 	=>	$posts_per_page,
			    	'orderby'   		=> 'date',
			    	'order' 			=> 'DESC',
			    	'tax_query' => array(
					        array(
					            'taxonomy' => 'tax_alone_project',   // taxonomy name
					            'field' => 'term_id',           // term_id, slug or name
					            'terms' => $cat_in_s,
					            'operator' => 'NOT IN',                  // term id, term slug or term name
					        )
					    )
			    ) );
			    global $i;
			    $i = 0 ;
			    if ( $loop->have_posts() ) :
			        while ( $loop->have_posts() ) : $loop->the_post(); 
			        	$i = $i + 1;
			        ?>
						<?php get_template_part( 'template-parts/content', 'project-carosel' );?>
			        <?php 
			        endwhile;
			    endif;
			    wp_reset_postdata();
			?>
                </div>
        </section>

<?php
	return ob_get_clean();
}

add_shortcode('project_slide', 'project_slide_shortcode');
add_action('init','project_slide_shortcode');
add_action('init','project_slide_shortcode_kc');
// add_action('init','get_even_tax_list');

function get_project_terms_list() {

	$terms = get_terms('tax_alone_project', array(
      'hide_empty' => false,
  	) );

	$post_cat = array();
		if (!empty($terms) && !is_wp_error($terms)) {		
			foreach ( $terms as $value ) {
				$post_cat[$value->term_id] = ucwords(strtolower( $value->name ) ) . ' (Posts Count: '. $value->count .')';
			}
		} else {
			$post_cat[0] = esc_html__( 'No Categories found', 'alonefoundation' );
		}

  	return $post_cat ;
} //End get_terms_list()..

add_action('init','get_project_terms_list');
// print_r(get__terms_list());
function project_slide_shortcode_kc() {
	$get_term_list = get_project_terms_list();
	if (function_exists('kc_add_map')) 
	{ 
	    kc_add_map(
	        array(
	            'project_slide' => array(
	                'name' => 'Project Slider',
	                'description' => __('Project Slider', 'alonefoundation'),
	                'icon' => 'sl-paper-plane',
	                'category' => 'Alone Foundation',
	                'params' => array(
	                	'general' 	=> array(
							array(
								'name' => 'posts_per_page',
								'label' => 'Post Per Page',
								'type' => 'number_slider',  // USAGE RADIO TYPE
								'options' => array(    // REQUIRED
									'min' => 0,
									'max' => 15,
									'unit' => ' ',
									'show_input' => true
								),
								// 'value' =>3,
								'value' => '5',
								'admin_label'	=> true,
							),

							array(
	 							'name' 			=> 'cat_in',
	 							'label' 		=> esc_html__( 'Exclude Categories', 'alonefoundation' ),
	 							'type' 			=> 'multiple',
	 							'options'		=> $get_term_list,
	 							'description'	=> esc_html__('Choose if You Want to Exclude Any Post Category, Control + Click to Select Multiple Categories to Exclude (No Categories are Excluded by Default)', 'codexin','alonefoundation'),
	 						),
        				),
	                	'Style' => array(
	    					array(
		                		'name' 		=> 'alone_css',
		                		'type'   	=> 'css',
		                		'options' 	=> array(
		                			array(
										"screens" 		=> "any,1199,991,767,479",
			                			'box'		=>array(
			                				array('property' => 'margin', 'label' => 'margin'),
			                				array('property' => 'padding', 'label' => 'padding'),
			                			),	
		                			)
		                		)// end of options 
							)
	                	) // style end 
	            	),  // End of params 
	        	)// End alonefoundation title
	    	)
	    ); // End add map
	} // End if
}